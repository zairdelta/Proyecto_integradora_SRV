...
const sensor_humedad_aire=require("App/Models/sensor_humedad_aire")
mongoose
...
export default class NFCController {
  public async todo({response}: HttpContextContract) {
    const agg = await sensor_humedad_aire.aggregate([
        {
          '$sort': {
            '_id': -1
          }
...
console.log(agg,"aggre")
  ...
  public async store({request,response}: HttpContextContract) {
    ...
    console.log(userdata.valor)
     ...
     const userdata1={"humedad_aire":userdata}
     const user=await sensor_humedad_aire.create(userdata1);
     return response.created({
       status:true,
       data:user
     })
  ...
  console.log(params)
   const user= await sensor_humedad_aire.findOne({params})
  ...
  public async destroy({params,response}: HttpContextContract) {
    ...
    await sensor_humedad_aire.deleteOne({_id:borrar})
    .then(()=>console.log("conectado"),
    response.created({
      status:true,
    }))
